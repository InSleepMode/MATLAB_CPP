...
В рамках импортозамещения разработано программное обеспечение, позволяющее производить вычисления с матрицами.
...
Например, функция от трех аргументов `FUNC(x, y, z)` записывается как
```
(FUNC x y z)
``` 
А оператор сложения (сложение `A` и `B`) записывается как 
```
(+ A B)
```
Аргументами функций и операторов могут быть переменные, константы и другие функции. Например, выражение `FUNC(x, y, z) + FUNC(a, 1, 2)` записывается как 
```
(+ (FUNC x y z) (FUNC a 1 2))
```

Для имен переменных используются только буквы английского алфавита (большие и маленькие) и цифры (не может быть первой цифры в имени переменной). 
...
Если значение целое число, то выводится как целое число (не выводить знаменатель). Если значение рациональная дробь, то выводится `числитель` `/` `знаменатель`. 
...
Во второй и далее строках выводится построчно матрица, выравненная столбцами по правому краю.
